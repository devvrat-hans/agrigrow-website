 ...
 * 
 * A modal/drawer showing the list of users that a user follows.
 * Mobile-friendly with bottom sheet behavior on small screens.
 */

import { useState, useCallback, useEffect } from 'react';
import { IconX, IconSearch, IconUserCheck } from '@tabler/icons-react';
...
export interface FollowingModalProps {
  ...
  onClose: () => void;
  /** Phone number of the user whose following list to display */
  userPhone: string;
...
export function FollowingModal({
  ...
  const {
    ...
    removeFromFollowing,
  } = useFollowing(isOpen ? userPhone : null, {
  ...
  // Handle follow change (optimistic update for unfollow)
  const handleFollowChange = useCallback((targetPhone: string, isFollowing: boolean) => {
    if (!isFollowing) {
      // User was unfollowed - remove from the list
      removeFromFollowing(targetPhone);
    } else {
      // User was followed back - update status
      updateFollowingStatus(targetPhone, { isFollowing: true });
    }
  }, [updateFollowingStatus, removeFromFollowing]);
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center gap-2">
            <IconUserCheck className="w-5 h-5 text-gray-500 dark:text-gray-400" />
            <h2
              id="following-modal-title"
