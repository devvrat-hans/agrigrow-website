/**
 * Groups By Crop API Route
 * 
 * Returns groups that have a specific crop in their crops array.
 * 
 * GET /api/groups/by-crop/[cropName]
 * 
 * Authentication:
 ...
 * GET /api/groups/by-crop/[cropName]
 * 
 * Returns groups that have the specified crop in their crops array.
 * 
 * Path Parameters:
 * @param {string} cropName - The crop name to filter by (URL encoded)
 * 
 * Query Parameters:
 * @param {number} page - Page number (default: 1)
 * @param {number} limit - Results per page (default: 20, max: 50)
 * @param {string} sortBy - Sort by field (memberCount, createdAt, name)
 * 
 * Headers:
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Decode and normalize crop name
    const decodedCropName = decodeURIComponent(cropName);
    const normalizedCropName = normalizeCropName(decodedCropName);

    if (normalizedCropName.length < 2) {
      return NextResponse.json(
        { success: false, error: 'Crop name must be at least 2 characters' },
        { status: 400 }
      );
    }
    ...
    const sortBy = searchParams.get('sortBy') || 'memberCount';
    ...
    // Build query conditions - case-insensitive search in crops array
    const cropRegex = new RegExp(normalizedCropName.replace(/\s+/g, '\\s*'), 'i');
    ...
    // Get related crops (other crops from the same groups)
    const relatedCrops: string[] = [];
    if (groups.length > 0) {
      const allCrops = new Set<string>();
      groups.forEach(group => {
        const crops = group.crops as string[] || [];
        crops.forEach(crop => {
          const normalizedCrop = normalizeCropName(crop);
          if (normalizedCrop !== normalizedCropName) {
            allCrops.add(crop);
          }
        });
      });
      relatedCrops.push(...Array.from(allCrops).slice(0, 10));
    }

    return NextResponse.json({
      success: true,
      data: {
        crop: decodedCropName,
        normalizedCrop: normalizedCropName,
    ...
    console.error('Error fetching groups by crop:', error);
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to fetch groups by crop',
